...
\section{Assumptions}%解释多一些，以及假设的合理性和原因 
\noindent
...
\begin{table}[H]
        ...
        $\bar{T}$	&the average Time Headway vector in a road\\
        $F(t_{i})$ &the distribution function of $t_{i}$\\
		$\bar{V}$	&the variance of the speed of all vehicles in the ith section of the road \\    
		$V_{i}$	&the average speed of all vehicles in the road  \\
		$\alpha$ &congestion evaluation index \\
...
\end{equation}
\indent $V$ is a column vector for all vehicle speed, which unit is km/h. The initial value of $V$ is a random vector which follows normal distribution.  $T$ is the headway of all drivers or self-driving systems. And $K$ is all vehicles' types. The meaning and value of the headway will be given in \ref{headway}. There are two kinds of values in $k_{i}$, $0$ means self-driving car and $1$ means manual-driving car.

\subsubsection{The Integral Structure of Cellular Automata}
The model of Cellular Automata is mainly based on the following aspects\\
...
\indent The inflow of vehicles in a section of the road consists of two parts. The first part is the inflow of the starting position in the road. The second part is the inflow of cars from auxiliary road to the main road. \\
\indent The outflow of vehicles includes the outflow of the end position of the road and the outflow from the main road to auxiliary road.\\
...
\indent Time Headway (TH) is the time difference between the two adjacent vehicles passing through the same location. In general, the TH is a random value. But when people drive a car, the front car has an emergency brake or deceleration in a very short time, the following car driver can respond only after a certain reaction time. It is easy to cause traffic accidents if the distance between the two cars is two small. So the TH between the two cars needs to be greater than a minimum safety TH. The safety TH is depended on the driving habit of the driver.\\
...
\indent Then the ratio distribution of the headway of the car needs to be determined. It is known from the previous researches\upcite{TH} that, when there is a minimum safety TH and crowding, the shift negative exponential distribution(SNED) can well fit the distribution of the TH of a large number of vehicles.\\
\indent So the random variable follows the SNED model. It indicates,
\begin{equation}
\label{TH}
		F(t)=1-e^{-\frac{t-\tau }{T-\tau}}
\end{equation}
where $F(t)$ is the distribution function of TH, $T$ is the average TH in a road, $\tau $ is the minimum safety TH. \\ 

\noindent\textbf{The calculation of parameter $T$ \\}

In traffic flow theory, traffic flow density\\
\begin{equation}
\begin{split}
 K &= \frac{Q}{V}\\
...
\end{split}
\end{equation}
where $K$ is the traffic flow density, $Q$ is vehicle flow per unit time, $V$ is the average velocity of vehicles and $D_{a}$ is the average distance of vehicles.\\
...
\end{equation}
\indent It indicates the average distance is the reciprocal of the traffic flow. Plugging \ref{T} into the equation for \ref{TH} ,we obtain the distribution of TH.
\subsubsection{The generation of TH by SNED model}
\indent When the parameters $T, \tau $ in $F(t)$ are obtained, we need to produce a large number of data that follows the SNED.\\
\indent MATLAB can only produce a group of data following the common distribution. To produce a group of data following $ F(t_{i})=1-e^{-\frac{t_{i}-\tau}{T-\tau}},t_{i}>\tau $, we consider a random variable, 
\begin{equation}
...
\end{equation}
\indent It indicates $t$ follows the SNED $F(t_{0})$. Therefore, the conclusion is obtained that when $ u \sim U(0,1) $, the random variable $t=G(u)$ follows SNED.\\
...
\indent $P$ is a column vector including the location of all the vehicles (the unit of $P$ is mile). With the process of simulation, $P$ is a vector that varies orderly. So we only need to consider how to determine the initial value of $P$.\\
\indent In our CA model, vehicles are distributed across the whole road at the beginning of simulation. The road is segmented through the given data sheet. From the given data sheet, we know that the traffic flow is different in different section of the road. So the distribution of vehicles is not uniform.\\
\indent At the same time, the conclusion is obtained that the length of each section of the road is about 1 to 2 miles, and every road has about dozens of sections. So in one section of the road, the distribution has little influence on the whole distribution of vehicles. Therefore, we suppose that in every section of road, the distribution of the vehicles is uniform. \\
\indent From the given data sheet, we can get the start and end of every section. The proportion of traffic flow in the peak period accounts for 8\% of the whole traffic flow. By calculating, we can get the length of every section $l_{i}$ and the number of vehicles $n_{i}$ at some moment.\\
\indent We discuss the value of $n_{i}$ in two situation between the peak time and non-peak time of the vehicle. 
\begin{equation}
	n_{i}=\begin{cases}
	...
	\end{cases}
\end{equation}
where $v_{i}$ means the average speed of cars and $t_{i}$ is the length of the peak hour.\\
\begin{equation}
	p_i=(i-1)\cdot\frac{l_{n}}{n_{i}}+p_{s} \indent \indent	i=1,2,3,…n
\end{equation}
where $p_{s}$ is the start mile post.\\
...
\end{equation}
\indent Initially, We directly put $\Delta t$ as the coefficient of vehicle acceleration. But we find the range of T is very large. So we use tanh function which is widely used in Neural Network to normalize it.\\
\begin{figure}[H]
...
\begin{equation}
	\Delta V_{1}=K_{t}.*A
\end{equation}
where A is a vector of random $a_{i}$, and $a_{i}$ is the maximum acceleration of the $i_{th}$ vehicle with uniform distribution. The unit of $a_{i}$ is $m/s^{2}$ and $a_{i}\sim U(1.1,2.1)$.\\
\indent When the speed of the rear car is higher than that of the front car, the acceleration of the rear car will be reduced to prevent pileup. The velocity variation $\Delta V_{2}$ is caused by reducing acceleration in advance.
\begin{equation}
\begin{split}
...
\subsection{The model of generating K}

\subsection{The model of changes in the number of cars}
\label{changes of cars}
\subsubsection{Monte Carlo Method}
\indent Next, we consider the Monte Carlo method to simulate the change in the number of vehicles.\\
...
\indent From the known data, we can get the traffic inflow on each road section of the day. The value of net inflow or net outflows of cars is the difference between the traffic flow of two adjacent road sections.\\
\indent In the model of the cellular automata, the time to simulate one step is a second. So we should consider the net flow of cars per unit time as $\Delta n$. And the traffic conditions on the road divided into the peak time and non-peak time. We can get
\begin{equation}
	\Delta n=\begin{cases}
	...
	\end{cases}
\end{equation}
where i means the $i$th road section. \\
\indent We use the matrix $P_{c}$ to represent the probability distribution of all the road sections inflow and outflow. So the matrix can be expressed as
\begin{equation}
	P_{c}=[\Delta n_1, \Delta n_2,\Delta n_3,\cdots, \Delta n_n]
\end{equation}
...
\end{equation}
of which the length is the quantity of road sections as $n$. And the range of the random number is 0 to 1.\\
\indent The situation of vehicles flow is 
\begin{equation}
	\Delta n_{i}=\begin{cases}
	&0 \indent\indent\indent \quad\text{\,\,if \,\, $P_{c}[i] < R[i]$}\\ 
	&1 \indent\indent\indent \quad\text{\,\,if \,\, $P_{c}[i] > R[i]$} 
	\end{cases}
\end{equation}
\indent It means when the value of $P_{c}[i]$ is smaller than the value of $R[i]$, $\Delta n_{i}$ is assigned to 1. When the value of $P_{c}[i]$ is larger,  $\Delta n_{i}$ is assigned to 0.

\subsection{Model of evaluation criteria for the degree of congestion}
\label{evaluation criteria}
\subsubsection{Use $\bar{V}$ to evaluate the degree of congestion}
\begin{itemize}
...
\end{itemize}
\begin{itemize}
	\item{\textbf{When there is some congestion.} When there is a congestion on a road, the average velocity of the vehicles must be slow. When there is a congestion only in one part of the road, the velocity in this part must be much lower than the average velocity $ \bar{V} $. So the speed distribution must be not uniform. Therefore, the deviation between the speed and the average speed of each vehicle is large. }
\end{itemize}

\begin{itemize}
	\item{\textbf{How to evaluate the degree of congestion by $\bar{V}$.} The conclusion is obtained that the degree of congestion must be related with the average speed of the speed of all vehicles in the section of the road and the deviation between the speed and average speed of all the vehicles.}
\end{itemize}

\subsubsection{Use $\alpha$ to evaluate the degree of congestion}
\indent For some section of the road, at some moment $t$, the velocity of every vehicle can be obtained. So we can use $ \bar{V} $ as the first index to evaluate the degree of congestion. Therefore, the smaller $\bar{V}$ of a road is, the more serious the crowding the road is.

\begin{itemize}
	\item{\textbf{How to evaluate the degree of congestion by $\alpha$}. And we use the standard deviation of the speed of all the vehicles $S$ to evaluate the degree of congestion. However, the larger the average velocity is, the larger the absolute value of the deviation is. To represent the degree of the road more objectively, we use relative deviation $\alpha =\frac{S}{\bar{V}} $ as the congestion evaluation index. The larger $\alpha$ is, the more serious the crowding the road is.}
\end{itemize}
\subsubsection{Comprehensive evaluation by $\bar{V}$ and $\alpha$}

\indent  Our evaluation criteria for the degree of congestion includes: $\bar{V}, \alpha $. More crowding road must have a smaller $\bar{V}$ and a larger $alpha_{i}$.\\
...
\indent We use $\bar{V}, \alpha$ index to evaluate the degree of congestion. The number of lanes of roads increases 1,2,3 respectively. And a simulation within 60 minutes is implemented.\\
\indent The figure below shows the variance of $\bar{V}$ and $\alpha$ within 60 minutes when different number of lanes of roads is added.\\
\begin{figure}[H]
	\centerline{\includegraphics[height=5cm]{lanes.eps}}
	\caption{the influence of adding new lane}	
\end{figure}
\indent From the above figure, when the number of lanes of roads increases, $\bar{V}$ will remarkably increase, and the value of $\alpha$ will remarkably decrease. And it means that when the number of lanes increases, there will be less congestion, and the road will be more smooth. 
...
\subsection{The effects of percentage of self-driving in Peak and Non-Peak travel time}
...
When different proportion of self-driving are set, we use the evaluation criteria $\bar{V}, \alpha$ in \ref{evaluation criteria} to evaluate the degree of congestion. \\
\indent When the proportion of self-driving is $0\%, 10\%, 50\%, 90\%$, the image of variations of $\bar{V}$ and $\alpha$ within 60 minutes are obtained respectively.\\
\indent The figure below shows within the peak travel hours, the variations of $\bar{V}$ and $\alpha$ within 60 minutes.\\
\begin{figure}[H]
...
\end{figure}
\indent The above figure shows in peak travel hours or non-peak hours, when the proportion of self-driving increases, the $\bar{V}$ will increase and the $\alpha$ will decrease, which indicates the congestion will decrease with the proportion of self-driving increasing.\\
\indent And when proportion transfers from $50\% $ to $90\% $, the variation is remarkable. When the proportion reach $90\%$, the $\bar{V}$ approaches the maximum allowed speed 60 miles per hour. So the operation efficiency of the road approaches maximum value. And there is little congestion occurred. \\
...
\indent We mainly use two indexes of $\bar{V}$ and $\alpha$ to analyze the model. So taking advantage of the rule of our cooperative relationship, we simulate the cellular automaton under the different percentage of the self-driving cars for 4 roads in one hour. 
\begin{figure}[H]
...
\indent We can know from the figure that $\bar{V}$ has increased and $\alpha$ has decreased when adding the cooperation rules. And different roads have the similar situations.\\
...
\indent Equally, the overall impact of the $\bar{V}$ and $\alpha$ about the cooperation rules is slightly small as the self-driving vehicle can accurately control the speed of itself.
...
\indent It's our honor to help you with analyzing the effects of allowing self-driving, cooperating vehicles on capacity of highways in the Greater Seattle area. We are writing this letter to report our latest ﬁndings. \\
...
\indent We believe our model is useful for management of self-driving cars in the future. You're welcome to contact us at any time for further cooperation. Yours Sincerely, Team \# 55869
